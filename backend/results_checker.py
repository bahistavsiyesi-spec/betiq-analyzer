...
def get_fixture_result_footballdata(home_team, away_team, match_date):
    ...
    try:
        ...
        if not home_parts or not away_parts:
            return None
        home_lower = home_parts[0]
        away_lower = away_parts[0]
...
def calculate_outcomes(analysis, home_score, away_score, ht_home_score=None, ht_away_score=None):
    ...
    try:
        ...
        if len(parts) == 2:
            score_correct = (int(parts[0]) == home_score and int(parts[1]) == away_score)
        else:
            score_correct = False
    ...
    if ht_home_score is not None and ht_away_score is not None:
        ...
        actual_ht2_over15 = ht2_total > 1.5
...
def send_result_to_telegram(analysis, home_score, away_score, outcomes, ht_home_score=None, ht_away_score=None):
    ...
    if high_conf:
        line_1x2 = f"{tick(outcomes['pred_1x2_correct'])} 1X2: {pred_text} → <b>{outcomes['actual_1x2']}</b>"
    else:
    ...
    if ht_home_score is not None and ht_away_score is not None:
        ...
        if ht_pct >= 65:
            ht_line = f"\n{tick(outcomes['ht_correct'])} İY 0.5 Üst (%{ht_pct}) → <b>{ht_result}</b>"
        else:
            ht_line = f"\n— İY 0.5: Oynanmadı 🚫"
